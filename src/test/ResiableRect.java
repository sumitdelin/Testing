...
public class ResiableRect extends JPanel {
...
//        g2.fill(s);
    ...
    class ShapeResizeHandler extends MouseAdapter {
        ...
        public void mouseDragged(MouseEvent event) {
            ...
            if (pos == 0) {
                if (points[1].getX() > (event.getPoint().x + 15) & points[1].getY() > (event.getPoint().y + 15)) {
            ...
            if (pos == 1) {
                if (points[0].getX() < (event.getPoint().x - 15) & points[0].getY() < (event.getPoint().y - 15)) {
                    points[pos].setRect(event.getPoint().x, event.getPoint().y, points[pos].getWidth(),
                            points[pos].getHeight());
                }
            }
